...
FROM
  master.map as map
  LEFT JOIN transactions.villages as villages ON map.id = villages.id
WHERE
  villages.id IS NULL;

/* user (user_id 1) is selecting location 4*/
INSERT INTO
  transactions.villages (name, population, owner_id, position_id)
VALUES
  ('rome', 50, 1, 4);
...
INSERT INTO
  transactions.crops (village_id, position_id, level)
VALUES
  (1, 1, 0),
  (1, 2, 0),
  (1, 3, 0),
...
INSERT INTO
  transactions.ressources (village_id, ressource_id, amount)
VALUES
  (1, 1, 1000),
  (1, 2, 1000),
  (1, 3, 1000),
...
FROM
  ...
  LEFT JOIN master.crop_upgrades as upgrades ON positions.ressource_type = upgrades.ressource_type_id
  LEFT JOIN master.crop_upgrades_cost as upgrades_cost ON upgrades.id = upgrades_cost.crop_upgrades_id
  LEFT JOIN transactions.ressources as user_ressouces ON user_ressouces.village_id = crops.village_id
  AND user_ressouces.ressource_id = upgrades_cost.ressource_type_id
WHERE
  crops.village_id = 1
  AND crops.position_id = 9
  AND upgrades.level = crops.level + 1;
...
FROM
  ...
  LEFT JOIN master.map as map ON villages.position_id = map.id
WHERE
  villages.owner_id = 1
...
SELECT
  ...
  y_pos,
  SQRT(
    POWER((x_pos - (-1)), 2) + POWER((y_pos - (-1)), 2)
  ) AS distance
FROM
  master.map
  LEFT JOIN transactions.villages as villages ON map.id = villages.id
...
FROM
  transactions.crops as crops
  LEFT JOIN master.crop_positions as pos ON crops.position_id = pos.id
  LEFT JOIN master.crop_production as prod ON prod.ressource_type_id = pos.ressource_type
  AND crops.level = prod.level
WHERE
  village_id = 1
...
FROM
  transactions.crops as crops
  LEFT JOIN master.crop_positions as pos ON crops.position_id = pos.id
  LEFT JOIN master.crop_production as production ON crops.level = production.level
  AND pos.ressource_type = production.ressource_type_id
WHERE
  crops.village_id = 1
GROUP BY
  ressource_type;
